...
def arg_parser():
	...
	parser.add_argument("--record", action="store_true", help="Record grasps as videos.")
	parser.add_argument("--max-record", help="Maximum number of grasps to record.", type=int, default=3)
	parser.add_argument("--output-dir", help="Directory to save recorded videos.", type=str, default="recorded_grasps")
...
def record_grasp_execution(env, gripper_6dof_pose, init_joint_state_genes, shake, fps=30):
	"""Record grasp execution"""
	model = env.mj_client.model
	data = env.mj_client.data
	
	frames = []
	start_time = [None]  # Use list to track start time
	
	# Setup camera - zoom in closer
	camera = mujoco.MjvCamera()
	camera.lookat[:] = [0, 0, 0]
	camera.distance = 0.8  # Closer (was 1.5)
	camera.azimuth = 90
	camera.elevation = -20
	
	with mujoco.Renderer(model, height=480, width=640) as renderer:  # Higher resolution
		# Setup grasp
		env.reset()
		...
		original_step = env.mj_client.step
		
		def step_and_capture():
			original_step()
			# Capture frame based on time, not every step
			elapsed = data.time - start_time[0]
			if len(frames) < elapsed * fps:
				renderer.update_scene(data, camera=camera)
				frames.append(renderer.render())
		
		# Replace step temporarily
		env.mj_client.step = step_and_capture
		
		# Close gripper
		env.close_gripper()
		
		# Apply shaking if requested
		if shake:
			rg_db.apply_all_gripper_shaking_debug(env)
		
		# Restore original step
		env.mj_client.step = original_step
		
		# Hold for longer to see final result
		for _ in range(100):
...
def replay_all_6dof_poses(env, all_6dof_pose_data, display_flags, recording_flags=None):
	robot_id = env.robot_id
	...
	# Setup recording
	recorded_count = 0
	if recording_flags and recording_flags['record']:
		os.makedirs(recording_flags['output_dir'], exist_ok=True)
		max_record = recording_flags['max_record']
	...
	for scs_ind_id in all_all_6dof_pose_ids:
		...
		# Recording mode
		if recording_flags and recording_flags['record'] and recorded_count < max_record:
			print(f"Recording grasp {recorded_count + 1}/{max_record}...")
			frames = record_grasp_execution(
				env=env,
				gripper_6dof_pose=gripper_6dof_pose,
				init_joint_state_genes=init_joint_state_genes,
				shake=display_flags['shake'],
				fps=60
			)
			
			output_path = os.path.join(
				recording_flags['output_dir'],
				f"grasp_{scs_ind_id}_fitness_{fitness:.3f}.mp4"
			)
			video = np.array(frames)
			media.write_video(output_path, video, fps=30)
			print(f"Saved to {output_path}")
			
			recorded_count += 1
			
			if recorded_count >= max_record:
				print(f"Reached maximum recording limit ({max_record})")
				break
			continue
...
def replay_6dof_poses(env, all_6dof_pose_data, display_flags, recording_flags=None):
	if display_flags['i_ind2display'] is not None:
		...
		replay_all_6dof_poses(
			...
			all_6dof_pose_data=all_6dof_pose_data,
			display_flags=display_flags,
			recording_flags=recording_flags
		)
...
def replay_grasps(run_folder_path, display_flags, recording_flags=None):
	# Don't open viewer if recording
	display = not (recording_flags and recording_flags['record'])
	env, all_6dof_pose_data, display_flags = init_replay_6dof_grasp_poses(
		run_folder_path, display_flags, display=display
	)
